...
form.addEventListener("submit", function(e) {
    ...
    if (username === "") {
        ...
        if (regex.test(username) === false) {
            errorMessages += "Please enter a valid username (letters, numbers, and spaces only).<br>";
        }
    }
    ...
    if (password === "") {
        ...
        if (regex.test(password) === false) {
            errorMessages += "Please enter a valid password (at least 8 characters, including at least one lowercase letter, one uppercase letter, and one number).<br>";
        }
    }
